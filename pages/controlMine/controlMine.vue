  ...
  <view class="controlMine-box">
    <view class="head">
      <uni-row class="user-head">
        <uni-col :span="4" class="img-logo">
          <img :src="logoImg" alt="" />
        </uni-col>
        <uni-col :span="15" class="name-code" style="padding: 10px 0 0 15px">
          <view>
            {{ mineData.name }}
          </view>
          <view> 您好，您的邀请码: {{ mineData.code }} </view>
        </uni-col>
        <uni-col :span="5" class="role" style="padding-top: 10px">
          <text style="float: right"> {{ mineData.role }} </text>
        </uni-col>
      </uni-row>
      <uni-row class="money">
        <uni-col :span="8" class="money-balance">
    ...
    <view class="nav">
      <uni-row class="nav-list">
        <uni-col
          ...
          :key="index"
        >
          <functionItem :functionData="functionData" class="nav-list-number">
          </functionItem>
        </uni-col>
      </uni-row>
    </view>
    <button class="btn-logout" type="default" @click="logOff">退出登录</button>
  </view>
</template>

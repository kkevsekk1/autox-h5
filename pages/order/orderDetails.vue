<template>
  <view class="orderDaeails">
      ...
      <view class="allclass content-orderDetail">
        ...
        <view class="content-orderDetail-allclass">
          <uni-row>
            <uni-col :span="14">
              <uni-row>
                <uni-col :span="6" style="max-width: 50px; max-height: 50px">
                  <img style="width: 100%" :src="imgLogo" alt="" />
                </uni-col>
                <uni-col :span="18">
                  <view>商品名</view>
          ...
          <uni-row
            v-for="(orderItem, index) in items.orderItems"
            :key="index"
          ...
          >
            <uni-col :span="14">
              <uni-row>
                ...
                <uni-col :span="18" style="padding-left: 3px">
    ...
    <view class="footer">
      ...
      <uni-row :gutter="20" v-if="items.status == '待支付'">
        <uni-col :span="12">
          <view class="row-btn-cancel" @click="cancelOrder">取消订单</view>
        </uni-col>
        <uni-col :span="12">
          <view class="row-btn-pay" @click="toPay">去支付</view>
        </uni-col>
      </uni-row>
      <view
    ...
    <uni-popup ref="popupService" type="bottom" background-color="#fff">
      <view class="popupService">
        <view class="popupService-title">
          <text>客服热线：</text>
          <text selectable="true" user-select="true">13523230214</text>
        </view>
        <uni-row :gutter="20">
          <uni-col :span="12">
            <view
              class="row-btn-cancel"
              style="background-color: #999; color: #fff"
              @click="$refs.popupService.close()"
              >取消</view
            >
          </uni-col>
          <uni-col :span="12">
            <view class="row-btn-pay" @click="dialPhone">拨打</view>
          </uni-col>
        </uni-row>
      </view>
    </uni-popup>
